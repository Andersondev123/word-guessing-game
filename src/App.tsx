...
export default function App(){
  ...
  const [challenge, setChallenge] = useState<Challenge | null>(null)
    ...
    function handleConfirm(){
      ...
      if(exists){
        alert("Você ja utilizou a letra " + value)
        setLetter("")
        return 
      }
      ...
      <main>
        ...
        <div className={styles.word}>
        {
          challenge.word.split("").map((letter, index)=>{
            ...
            return(
              <Letter key={`${letter}-${index}`} value={letterUsed?.value} color={letterUsed?.correct ? "correct" : "default"}/>
            )
          })
        }
        </div>
        ...
        <div className={styles.guess}>
          <Input value={letter} autoFocus maxLength={1} placeholder="?" onChange={(e) => setLetter(e.target.value)}/>
